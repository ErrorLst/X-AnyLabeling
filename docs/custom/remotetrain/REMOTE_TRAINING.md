...
**输入控件的呈现口径（写死）**：数值框（`QSpinBox` / `QDoubleSpinBox`）一律
`setButtonSymbols(NoButtons)`，**不显示上下步进按钮**；参数表单里的每一个
输入控件（数值框、下拉框、可编辑下拉框）的 `wheelEvent` 一律
`event.ignore()`：滚轮**不改变取值**，事件也**未被 accept**，继续向上传播
⇒ 滚轮滚动整页（`QScrollArea`），避免滚动页面时误改参数。取值只能靠键盘
输入（含输入法）与下拉选择：方向键仍切换下拉项、键盘输入的数字仍受
`setRange` 约束、可编辑下拉框仍能输入数字。控件仍是 `QComboBox` /
`QSpinBox` / `QDoubleSpinBox` 的子类（`isinstance` 类型判断与提交语义
不变，§5.2.2）。
