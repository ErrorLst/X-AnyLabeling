...
`anylabeling/custom/smudge_tool/`（4 个文件 2747 行：`texture_fill.py` 算法 598 行、
`operations.py` 读写/备份/几何 516 行、`smudge_filter.py` Qt 层 1570 行、
`__init__.py` 导出）；
测试 `tests/custom/smudge_tool/`（6 个文件 3884 行，含写回编码参数、TIFF
...
- **外边界权重：边界处贴片占优，向内淡出**：ROI 最外圈的新鲜贴片权重是
  `port + (1 - port) * feather`，`texture_fill.edge_feather`（块间羽化
  `feather_alpha` 的**补集**，两者相加恒为 1）在最外一圈 ≈1（`r = 4` 时 0.962），
  向内 `r` 像素回落到端口原有的权重。**方向不能反**：把权重乘上 `(1 - feather)`
  会让最外圈权重趋 0，也就是把被替换的像素（缺陷）原样留在圈上，一条接缝变两条
  （旧版实测圈均值 26.4/22.5，缺陷色 20；跳变 79.8/112.9）。未写过的像素 `port`
  本来就是 1，羽化对「第一次写入」的圈像素是恒等变换——**接缝的消除靠的是源窗口
  放大，不是羽化**（见下一条），羽化只决定重叠带上谁说了算。
  宽度 `r = edge_ramp_for(高, 宽) = clamp(round(min(高, 宽) / 10), 0, 8)`
  （`50×40` 的框 → 4px、短边 50 → 5px、短边 ≥80 → 8px）。`fill_roi` 在两种场合把
  自动宽度压成 0：短边 ≤ 10 像素；以及框内有块装不下「块 + 边距」、整框由「对齐
  贴片」1:1 取源窗口像素时——后者羽化会破坏「贴片原样」这个承诺。窗口装得下
  「块 + 边距」的其余小框走正常匹配（见「已知坑」），同样取 0：保住 `r == 0` 的
  逐字节等价（本轮实测 48 组 dtype × 通道 × 形状组合全部逐字节相同）。
  宽度可经 `fill_roi(..., edge_ramp=...)` 显式指定（默认 `None` = 按尺寸自适应），
  Qt 层不传、永远用自适应值。
- **源点在框内：提示且不处理**：判定 = 源点取整后落在半开 ROI 内
  ...
  磁盘字节、`_history`、`_backups`、光标与 `_source_box` 都不动。守卫只能认「源点
  本身」：源窗口**必含**源点（居中后整体 `np.clip` 平移、绝不裁短），而「源点在框外、
  窗口仍与框重叠」是常态路径（窗口是框的 3 倍，源点离框 30 像素就必然重叠），两者
  在几何上不可区分，不能整体拦下。源点在框内 = 让算法到缺陷自己身上取纹理，按约定
  直接拒绝并提示；命中时窗口其实覆盖到框外，但这条守卫是策略、不是几何必然。
- **源窗口 = 框的 3 倍**（`operations.SOURCE_WINDOW_FACTOR`）：窗口仍以源点为
  中心、受图像边界整体平移（`np.clip` 平移、绝不裁短），只是宽高各放大到 ROI 的
  3 倍。理由是可复现的：1× 窗口下匹配窗与搜索区高度相同，代价图只剩**一行**
  （50×40 的框，最外圈一个整块 40×40 匹配窗 → 1×11 个落点），最小值被挤到区间
  边缘（实测 12 个块全落到同一个 `(118,153)`，离对齐位置 68 行 93 列），接缝跳变
  57.25；放大到 3× 后代价图 81×121，能找到与周围同灰阶的落点（实测其中一块代价
  0.0），跳变 6.5~8.3。
  窗口与框相交不是错误：`_forbid_roi` 逐候选剔除会复制 ROI 自身的贴片（实测
  3× 窗口下 9801 个落点里剔除 2184 个），其余照常；窗口小到装不下「块 + 边距」时
  `fill_roi` 把边距逐级减到 0（退回对齐贴片），连匹配窗都装不下时退回**全图搜索**
  （`search = work`，`_forbid_roi` 按图坐标判定）。
- **小框填充（对齐贴片）**：匹配窗口里没有任何已确定像素时（图像比「块 + 边距」还小、
  或压根没有源窗口）这类块不被跳过（跳过会留下缺陷像素），而是按「块在 ROI 内的
  相对偏移」从源窗口同位置取同尺寸贴片。贴片落回 ROI 自身（源窗口与框重叠、对齐
  位置落在框内）、源窗口装不下对齐贴片、或根本没有源窗口时仍然跳过；这类块同样走
  `_blend_block`。**源点落在框内**这一情形由上面的守卫在更早处拦下并提示，不再落进
  「未发生变化」。**行为变化（本轮）**：窗口放大后，10×10 这类小框的块周围有了
  8 像素已知边距，于是走正常匹配而不再 1:1 盖源点纹理，
  `test_a_small_drag_fills_the_region_and_writes_the_file` 随之改写。
...
（需 PyQt6 + numpy + OpenCV；本工作区用仓库里的 `.venv`，全套 200 个用例：
`test_st_operations.py` 29（本轮 +1：源窗口默认 3×，几何两条显式传 `factor=1`）+
`test_st_texture_fill.py` 64（本轮 +2：渐变与颗粒材质两条「1× 窗口有接缝、3× 窗口
接缝消失、最外圈均值 = 材质真实值且远离缺陷色 20」的统计 / 边界权重对未写像素恒等、
对已写像素接管 / 羽化宽度区间表 / 补集关系与形状 dtype / 贴角框四边都变化 /
uint8·uint16 × 1·3·4 通道 / `edge_ramp=0` 即旧基准）+ 
`test_st_filter.py` 54（本轮 1 条既有用例改写：小框走匹配不再走对齐贴片；其余
源点在框内零痕迹、Ctrl+Z 撤的是上一步正常填充、源点正好在右·下边界上不被拦、
源点在框外但窗口与框相交照常执行都没动）+
...
- **源窗口放大后，小框不再 1:1 盖源点纹理**：ROI 短边小于一个纹理块时，旧路径是
  从同尺寸源窗口取对齐贴片；窗口放到 3× 之后块周围有了已知边距，改走正常匹配。
  代价是「小框 = 把源点处的同尺寸纹理盖过来」这条承诺只在图像小到装不下
  「块 + 边距」时仍然成立。要恢复旧行为，把调用处的
  `operations.SOURCE_WINDOW_FACTOR` 传 1。
- **单调渐变的大图仍有接缝（本轮自报的残留）**：材质本身没有同灰阶延续时（单调
  渐变），源点离框较远的大图仍可能在框边出现接缝——正常匹配也会选中另一灰阶的
  贴片。根因是端口 `gain=False`：它不做增益/偏置补偿，贴片照抄源像素的灰度，
  所选窗口又以源点为中心、只有框的 3 倍，够不到远处同灰阶的落点。实测（400×300
  图、框 50×40）：源点距框 60px 跳变 14.55、150px 跳变 82.50，高于本轮测试自
  立的 ≤10 / 材质 + 8 门槛；同构造成颗粒材质 9.4~10.2 达标。缓解办法是把源点选
  得离框更近。框最外圈不残留缺陷这一条不受影响：任何场景都不残留（实测
  89.9~98.9，真值 96.65）。
- **外边界权重对「第一次写入」的圈像素是恒等变换**：`port` 对未写过的像素本来就是
  1，`port + (1 - port) * feather` 因此还是 1，缺陷不会残留在圈上；接缝的消除靠
  源窗口放大，羽化只决定重叠带上谁说了算。旧版把权重乘 `(1 - feather)` 会保留被
  替换的像素，已被本轮修正（回归：圈均值必须接近材质真实值，不得接近缺陷色）。
