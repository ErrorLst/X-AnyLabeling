"""Shared fixtures for the reset view on switch tests.
...
Two kinds of widget are built here. The wrapper tests use a fake widget
carrying the same state names as the real one (zoom_widget, zoom_mode,
zoom_values, scroll_values, actions, filename), while the tests of the
mount point build a real offscreen LabelingWidget on the shipped
configuration.
"""
...
#: Sizes of the images the real widget tests load, smallest first.
IMAGE_SIZES = ((320, 200), (640, 480), (1024, 768))
...
def _probe_writable(directory: str) -> None:
    ...
    try:
        trash = trash_root()
        os.makedirs(trash, exist_ok=True)
        shutil.move(probe, os.path.join(trash, _stamp() + "-" + PROBE_NAME))
    except OSError:
        pass


def write_image(path: str, size=(320, 200)) -> str:
...
@pytest.fixture
def rvs_scratch():
    "System temp scratch directory holding the images of one test."
...
@pytest.fixture
def rvs_images(rvs_scratch):
    "Return paths of three real PNG images of different sizes."

    paths = []
    for index, size in enumerate(IMAGE_SIZES):
        name = "rvs_image_%d.png" % index
        paths.append(write_image(os.path.join(rvs_scratch, name), size))
    return paths
